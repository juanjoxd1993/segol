			...
			<table class="pdf-head" cellpadding="0" cellspacing="0" border="0">
				<tr>
					<td>
						<div class="logo">
							@if ( $obj->company_id == 1 )
								<img src="{{ asset('backend/img/punto_gas_logo.jpeg') }}" alt="">
							...
							<table class="pdf-totals" cellspacing="0" cellpadding="0" border="0" align="right">
								<tbody>
									<tr>
										...
										<td>{{ number_format($obj->taxed_operation, 2, '.', '') }}</td>
									</tr>
									<tr>
										<td>IGV (18.00)</td>
										<td>{{ $obj->currency_symbol }}</td>
										<td>{{ number_format($obj->igv, 2, '.', '') }}</td>
									</tr>
									<tr>
										<td>Op. Inafecta</td>
										<td>{{ $obj->currency_symbol }}</td>
										<td>{{ number_format($obj->unaffected_operation, 2, '.', '') }}</td>
									</tr>
									<tr>
										<td>Op. Exonerada</td>
										<td>{{ $obj->currency_symbol }}</td>
										<td>{{ number_format($obj->exonerated_operation, 2, '.', '') }}</td>
									</tr>
									<tr>
										<td>Importe Total</td>
										<td>{{ $obj->currency_symbol }}</td>
									...
									@if ( $obj->igv_perception && $obj->igv_perception > 0 )
										<tr>
											<td>Percepción {{ $obj->igv_perception_percentage * 1 }}%</td>
											<td>{{ $obj->currency_symbol }}</td>

											@if ($obj->voucher_type_id_original == 14)
												<td>{{ number_format($obj->igv_perception * $obj->total, 2, '.', '') }}</td>
											@else
												<td>{{ number_format($obj->igv_perception, 2, '.', '') }}</td>
											@endif
										</tr>
										<tr>
											<td>Total a Pagar</td>
											<td>{{ $obj->currency_symbol }}</td>
											<td>{{ number_format($obj->total_perception, 2, '.', '') }}</td>
										</tr>
									@endif
								</tbody>
							</table>
