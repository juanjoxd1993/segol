<template>
    <!--begin::Modal-->
    <div class="modal fade" id="abastecimiento-register-modal" tabindex="-1" role="dialog"
...
export default {
    ...
    watch: {
        'model.quantity': function (val) {
            ...
            if (Number(val) > Number(this.stock)) {
        ...
        'model.article_id': function (val) {
            if (val != '' && ((Number(this.model.quantity) > 0 && Number(this.model.quantity) <= Number(this.stock)))) {
                document.getElementById('add_article_2').disabled = false;
            } else {
                document.getElementById('add_article_2').disabled = true;
            }
    ...
    created() {
        EventBus.$on('abastecimiento-register-modal', function (articles, movement_class_id, movement_type_id) {
            this.articles = articles;
            this.movement_class_id = movement_class_id;
            this.movement_type_id = movement_type_id;
            $('#abastecimiento-register-modal').modal('show');
        }.bind(this));

        EventBus.$on('abastecimiento-register-modal_hide', function () {
    ...
    mounted() {
        this.newSelect2();
        $('#abastecimiento-register-modal').on('hide.bs.modal', function (e) {
            this.model.article_id = '';
            this.model.quantity = '';
            $('#article').val(null).trigger('change');
            // document.getElementById('add_article_2').disabled = false;
