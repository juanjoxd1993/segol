    ...
    <div>
        ...
        <div class="kt-portlet" v-if="show_table">
            ...
            <div class="kt-portlet__body kt-portlet__body--fit">
                <!--begin: Datatable -->
                <div class="kt-datatable-register-document-charge"></div>
                <!--end: Datatable -->
            </div>
    ...
    export default {
        ...
        data() {
            return {
                show_table: false,
				model: {},
                register_document_charge_datatable: undefined,
				items: [],
            }
        },
        created() {
        ...
        mounted() {
            EventBus.$on('secondStateUpdate', function(response) {
				...
				Vue.nextTick(function() {
					if ( vm.register_document_charge_datatable == undefined ) {
						vm.fillTableX();
					} else {
						vm.register_document_charge_datatable.originalDataSet = vm.items;
						vm.register_document_charge_datatable.load();
					}
				
					vm.register_document_charge_datatable.on('kt-datatable--on-ajax-done', function() {
        ...
        watch: {
            items: function (val) {
				if ( val.length > 0 ) {
					...
					this.register_document_charge_datatable.originalDataSet = this.items;
					this.register_document_charge_datatable.load();

					this.register_document_charge_datatable.on('kt-datatable--on-ajax-done', function() {
						EventBus.$emit('loading', false);
					});
				}
			}
        ...
        methods: {
            ...
            fillTableX: function() {
                ...
                this.register_document_charge_datatable = $('.kt-datatable-register-document-charge').KTDatatable({
                ...
                // this.register_document_charge_datatable.columns('subgroup_id').visible(false);
            },
			saveDocumentCharge: function() {
				...
				axios.post(this.url, {
					...
					this.model = {};
					this.register_document_charge_datatable.destroy();
					this.register_document_charge_datatable = undefined;
					this.items = [];
