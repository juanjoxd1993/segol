    ...
    export default {
        ...
        mounted() {
            EventBus.$on('show_table', function(response) {
                this.company_id = response.data;
                if ( this.datatable == undefined ) {
                    this.fillTableX();
                } else {
                    this.datatable.setDataSourceParam('company_id', this.company_id);
                    this.datatable.load();
                }
        ...
        methods: {
            openModal: function() {
                EventBus.$emit('create_modal', this.company_id);
            },
            fillTableX: function() {
                let vm = this;
            ...
            manageActions: function(event) {
                ...
                } else if ( $(event.target).hasClass('edit-address') ) {
                    event.preventDefault();
                    let client_id = $(event.target).parents('tr').find('td[data-field="id"] span').html();
                    EventBus.$emit('loading', true);
                    EventBus.$emit('address_modal', client_id);
                } else if ( $(event.target).hasClass('price-list') ) {
                    event.preventDefault();
                    let client_id = $(event.target).parents('tr').find('td[data-field="id"] span').html();
                    EventBus.$emit('loading', true);
                    EventBus.$emit('price_list_modal', client_id);
                }
            },
        }
    };
