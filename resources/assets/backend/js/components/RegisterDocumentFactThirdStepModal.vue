		...
		<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
			<div class="modal-content">
				...
				<form class="kt-form" @submit.prevent="formController(url, $event)">
					...
					<input type="hidden" name="article_name" id="article_name" v-model="model.article_name">
							  	...
							  	<div class="col-lg-4">
									<div class="form-group">
										...
										<select class="form-control" name="article_id" id="article_id" v-model="model.article_id" @focus="$parent.clearErrorMsg($event)">
											...
											<option v-for="article in articles" :value="article.id" v-bind:key="article.id">{{ article.name }}</option>
										</select>
	...
	export default {
		...
		methods: {
			...
			formController: function(url, event) {
				...
				let article = this.articles.find( element => element.id == this.model.article_id );
				this.model.unit_name = unit ? unit.name : '';
				this.model.article_name = article ? article.name : '';
