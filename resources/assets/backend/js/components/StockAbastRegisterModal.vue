<template>
    <!--begin::Modal-->
    <div class="modal fade" id="stock-abast-register-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<script>
    ...
    export default {
        ...
        created() {
            EventBus.$on('stock-abast-register-modal', function(articles, movement_class_id, movement_type_id) {
                this.articles = articles;
                this.movement_class_id = movement_class_id;
                this.movement_type_id = movement_type_id;
                $('#stock-abast-register-modal').modal('show');
            }.bind(this));

            EventBus.$on('stock-abast-register-modal_hide', function() {
                this.model.article_id = '';
                this.model.quantity = '';
                this.model.quantity_2 = '';
                this.model.quantity_3 = '';
                $('#article').val(null).trigger('change');
                $('#stock-abast-register-modal').modal('hide');
                // document.getElementById('add_article_2').disabled = false;
            }.bind(this));
        ...
        mounted() {
            ...
            $('#stock-abast-register-modal').on('hide.bs.modal', function(e) {
                this.model.article_id = '';
                this.model.quantity = '';
                this.model.quantity_2 = '';
                this.model.quantity_3 = '';
