...
class SalesRegisterReportController extends Controller
{
    ...
    public function list() {
        ...
        $query = DB::select(DB::raw("SELECT company_id, client_id, client_name, voucher_type_id, serie_number, 
        voucher_number_since, voucher_number_to, credit_note_reference_serie, credit_note_reference_number, 
        issue_date, payment_id, taxed_operation, unaffected_operation, exonerated_operation, igv, total 
        FROM ( SELECT D.company_id, D.client_id, D.client_name, D.voucher_type_id, D.serie_number, 
        D.voucher_number AS voucher_number_since, D.voucher_number AS voucher_number_to, D.credit_note_reference_serie, 
        D.credit_note_reference_number, D.issue_date, D.payment_id, D.taxed_operation AS taxed_operation, 
        D.unaffected_operation AS unaffected_operation, D.exonerated_operation AS exonerated_operation, 
        D.igv AS igv, D.total AS total FROM vouchers AS D WHERE D.issue_date >= '".$since_date."' AND D.issue_date <= '".$to_date."' AND D.voucher_type_id <> 2 AND D.voucher_type_id <> 3 AND D.company_id = ".$company_id." 
        UNION SELECT D1.company_id, D1.client_id, D1.client_name, D1.voucher_type_id, D1.serie_number, min( D1.voucher_number ) AS voucher_number_since, max( D1.voucher_number ) AS voucher_number_to, D1.credit_note_reference_serie, D1.credit_note_reference_number, D1.issue_date, D1.payment_id,
         sum( D1.taxed_operation ) AS taxed_operation, sum( D1.unaffected_operation ) AS unaffected_operation, 
         sum( D1.exonerated_operation ) AS exonerated_operation, sum( D1.igv ) AS igv, 
         sum( D1.total ) AS total FROM vouchers AS D1 WHERE D1.issue_date >= '".$since_date."' AND D1.issue_date <= '".$to_date."' AND D1.voucher_type_id = 2 AND D1.ose <> 3  AND D1.company_id = ".$company_id." 
        GROUP BY D1.company_id, D1.client_id, D1.client_name, D1.voucher_type_id, D1.serie_number, D1.credit_note_reference_serie, D1.credit_note_reference_number, D1.issue_date, D1.payment_id 
        UNION SELECT D2.company_id, D2.client_id, D2.client_name, D2.voucher_type_id, D2.serie_number, D2.voucher_number AS voucher_number_since, D2.voucher_number AS voucher_number_to, D2.credit_note_reference_serie, D2.credit_note_reference_number, D2.issue_date, D2.payment_id, ( D2.taxed_operation * - 1 ) AS taxed_operation, ( D2.unaffected_operation * - 1 ) AS unaffected_operation, ( D2.exonerated_operation * - 1 ) AS exonerated_operation, ( D2.igv * - 1 ) AS igv, ( D2.total * - 1 ) AS total FROM vouchers AS D2 WHERE D2.issue_date >= '".$since_date."' AND D2.issue_date <= '".$to_date."' AND D2.voucher_type_id = 3 AND D2.company_id = ".$company_id.") vouchers 
        GROUP BY company_id, client_id, client_name, voucher_type_id, serie_number, voucher_number_since, voucher_number_to , issue_date
       
        
        ORDER BY company_id, voucher_type_id, serie_number, voucher_number_since, voucher_number_to"));
         /* , payment_id, taxed_operation, unaffected_operation, exonerated_operation,igv, total,credit_note_reference_serie, credit_note_reference_number*/ 
    ...
    public function export() {
        ...
        UNION SELECT D1.company_id, D1.client_id, D1.client_name, D1.voucher_type_id, D1.serie_number, min( D1.voucher_number ) AS voucher_number_since, max( D1.voucher_number ) AS voucher_number_to, D1.credit_note_reference_serie, D1.credit_note_reference_number, D1.issue_date, D1.payment_id, sum( D1.taxed_operation ) AS taxed_operation, sum( D1.unaffected_operation ) AS unaffected_operation, sum( D1.exonerated_operation ) AS exonerated_operation, sum( D1.igv ) AS igv, sum( D1.total ) AS total FROM vouchers AS D1 WHERE D1.issue_date >= '".$since_date."' AND D1.issue_date <= '".$to_date."' AND D1.voucher_type_id = 2 AND  D1.ose <> 3 AND D1.company_id = ".$company_id." 
        GROUP BY D1.company_id, D1.client_id, D1.client_name, D1.voucher_type_id, D1.serie_number, D1.credit_note_reference_serie, D1.credit_note_reference_number, D1.issue_date, D1.payment_id 
        UNION SELECT D2.company_id, D2.client_id, D2.client_name, D2.voucher_type_id, D2.serie_number, D2.voucher_number AS voucher_number_since, D2.voucher_number AS voucher_number_to, D2.credit_note_reference_serie, D2.credit_note_reference_number, D2.issue_date, D2.payment_id, ( D2.taxed_operation * - 1 ) AS taxed_operation, ( D2.unaffected_operation * - 1 ) AS unaffected_operation, ( D2.exonerated_operation * - 1 ) AS exonerated_operation, ( D2.igv * - 1 ) AS igv, ( D2.total * - 1 ) AS total FROM vouchers AS D2 
        WHERE D2.issue_date >= '".$since_date."' AND D2.issue_date <= '".$to_date."' AND D2.voucher_type_id = 3 AND D2.company_id = ".$company_id.") vouchers 
        GROUP BY company_id, client_id, client_name, voucher_type_id, serie_number, voucher_number_since, voucher_number_to, credit_note_reference_serie, credit_note_reference_number, issue_date, payment_id 
        ORDER BY company_id, voucher_type_id, serie_number, voucher_number_since, voucher_number_to"));
