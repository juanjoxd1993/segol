...
class LiquidationController extends Controller
{
    public function index() {
        ...
        return view('backend.liquidations')->with(compact('companies', 'warehouse_document_types', 'payment_methods', 'currencies'));
    }
    ...
    public function getWarehouseMovements() {
        ...
        $elements = WarehouseMovement::select('id', 'movement_number', 'referral_guide_series', 'referral_guide_number', 'license_plate', 'created_at')
    ...
    public function list() {
        ...
        $returnWarehouseMovement = WarehouseMovement::select('id')
            ->where('referral_serie_number', $saleWarehouseMovement->referral_serie_number)
            ->where('referral_voucher_number', $saleWarehouseMovement->referral_voucher_number)
            ->whereIn('action_type_id', [5, 10])
            ->get();
        ...
        $movementDetails->map(function ($item, $index) use ($returnWarehouseMovement, &$sumConvertedAmount) {
            ...
            if ( isset($returnWarehouseMovement) ) {
                $sumConvertedAmount = 0;

                foreach ($returnWarehouseMovement as $itemReturn) {
                    $returnDetail = WarehouseMovementDetail::select('id', 'article_code', 'converted_amount')
                        ->where('warehouse_movement_id', $itemReturn->id)
                        ->where('article_code', $item->article_id)
                        ->first();

                    if ( isset($returnDetail) ) {
                        $sumConvertedAmount += $returnDetail->converted_amount;
                    }
                }

                // $item->return_warehouse_movement_id = $returnWarehouseMovement->id;
                $item->return_converted_amount = number_format($sumConvertedAmount, 4, '.', '');
                $item->balance_converted_amount = number_format($item->converted_amount - $sumConvertedAmount, 4, '.', '');
                $item->new_balance_converted_amount = number_format($item->converted_amount - $sumConvertedAmount, 4, '.', '');
            } else {
	...
	public function verifyDocumentType() {
		...
		$warehouse_movement = WarehouseMovement::find($warehouse_movement_id, ['id', 'created_at']);
		$warehouse_movement_created_at = date('Y-m-d', strtotime($warehouse_movement->created_at));
		...
		if ( $warehouse_document_type->previous_date_flag ) {
			...
			if ( $voucher ) {
				if ( $voucher->issue_date <= $warehouse_movement_created_at ) {
	...
	public function store() {
		...
		$warehouse_movement = WarehouseMovement::find($model['warehouse_movement_id'], ['id', 'referral_guide_series', 'referral_guide_number', 'scop_number', 'license_plate', 'state', 'created_at']);
		...
		foreach ($sales as $sale) {
			$client = Client::find($sale['client_id'], ['id', 'code', 'business_name', 'link_client_id', 'payment_id', 'credit_limit_days', 'credit_balance']);
			...
			if ( $sale['warehouse_document_type_id'] >= 4 && $sale['warehouse_document_type_id'] <= 9 ) {
				...
				foreach ($sale['details'] as $detail) {
					$article = Article::find($detail['article_id'], ['name', 'sale_unit_id']);
			...
			foreach ($sale['details'] as $index => $detail) {
				...
				$sale_detail->kg = $detail['quantity'] * $article->convertion;
