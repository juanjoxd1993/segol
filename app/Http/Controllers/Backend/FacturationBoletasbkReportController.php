...
class FacturationBoletasReportController extends Controller
{
	...
	public function list() {
		...
		$client_id = request('model.client_id');
			...
			->select('voucher_details.id', 'companies.short_name as company_short_name', 'issue_date', 'business_units.name as business_unit_name', 'client_zones.name as client_zone_name', 'client_sectors.name as client_sector_name', DB::Raw('CONCAT("R-", client_routes.id) as client_route_id'),'vouchers.serie_number', 'vouchers.voucher_number','voucher_details.name as name','quantity as sum_total',DB::Raw('(SELECT SUM(voucher_details.quantity * (SELECT articles.convertion FROM articles WHERE articles.id = voucher_details.article_id ))/1000 ) AS tm_total'),'voucher_details.original_price as price',DB::Raw('(SELECT SUM(voucher_details.total)) AS total'), 'clients.code as client_code', 'clients.business_name as client_business_name', DB::Raw('CONCAT(vouchers.referral_guide_series, "-", vouchers.referral_guide_number) as guide'),'ubigeos.district as district', 'ubigeos.province as province', 'ubigeos.department as department')
			...
			foreach ($elements as $voucherdetail) {
                ...
                $totals_total += $voucherdetail['total'];
			    $totals_tm_total += $voucherdetail['tm_total'];
                ...
                $voucherdetail->tm_total = $voucherdetail['tm_total'];
        ...
        $totals->tm_total = $totals_tm_total;
		$totals->client_code = '';
