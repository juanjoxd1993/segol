...
class BudgetController extends Controller
{
	...
	public function list() {
		...
		if ( $business_unit_id == 1 || $business_unit_id == 3 ) {
			$elements = ClientZone::select('id', 'name')
				->get();
		} elseif ( $business_unit_id == 2 ) {
			$elements = ClientSector::select('id', 'name')
				->get();
		}
	...
	public function store() {
		...
		foreach ($ids as $index => $id) {
			...
			if ( $business_unit_id == 1 || $business_unit_id == 3 ) {
				$budget->client_zone_id = $id;
			} elseif ( $business_unit_id == 2 ) {
				$budget->client_sector_id = $id;
			}
		...
		$channels_total_metric_tons = $budget_channels->sum('sum_metric_tons');
		...
		foreach ($channels as $channel) {
			...
			if ( $filtered ) {
				$percentage = ( $filtered->sum_metric_tons * 100 ) / $channels_total_metric_tons;
		...
		if ( $business_unit_id == 1 || $business_unit_id == 3 ) {
			...
			$budget_sectors = SaleDetail::join('sales', 'sale_details.sale_id', '=', 'sales.id')
				->leftjoin('clients', 'clients.id', '=', 'sales.client_id')
				->join('articles', 'articles.id', '=', 'sale_details.article_id')
				->where('sales.sale_date', '>=', $first_day)
				->where('sales.sale_date', '<=', $last_day)
				->where('clients.business_unit_id', $business_unit_id)
				->select('clients.sector_id as client_sector_id', DB::Raw('SUM((quantity * convertion) / 1000) AS sum_metric_tons, SUM(sale_details.total) AS sum_total'))
				->groupBy('clients.sector_id')
				->orderBy('clients.sector_id', 'asc')
				->get();

			$sectors_total_metric_tons = $budget_sectors->sum('sum_metric_tons');
			$sectors = ClientSector::select('id', 'name')->get();
			...
			foreach ($sectors as $sector) {
				...
				$element->days = $days;
				$element->business_unit_id = $business_unit_id;
				$element->client_sector_id = $sector->id;

				$filtered = $budget_sectors->first(function ($item, $key) use ($sector) {
					return $item->client_sector_id === $sector->id;
				});

				if ( $filtered ) {
					$percentage = ( $filtered->sum_metric_tons * 100 ) / $sectors_total_metric_tons;
					$element->metric_tons = ( $totalBudget->metric_tons * $percentage ) / 100;
					$element->total = ( $totalBudget->total * $percentage ) / 100;
					$element->percentage = $percentage;
				} else {
			...
			$budget_zones = SaleDetail::join('sales', 'sale_details.sale_id', '=', 'sales.id')
				->leftjoin('clients', 'clients.id', '=', 'sales.client_id')
				->join('articles', 'articles.id', '=', 'sale_details.article_id')
				->where('sales.sale_date', '>=', $first_day)
				->where('sales.sale_date', '<=', $last_day)
				->where('clients.business_unit_id', $business_unit_id)
				->select('clients.zone_id as client_zone_id', DB::Raw('SUM((quantity * convertion) / 1000) AS sum_metric_tons, SUM(sale_details.total) AS sum_total'))
				->groupBy('clients.zone_id')
				->orderBy('clients.zone_id', 'asc')
				->get();

			$zones_total_metric_tons = $budget_zones->sum('sum_metric_tons');
			$zones = ClientZone::select('id', 'name')->get();
			...
			foreach ($zones as $zone) {
				...
				$element->days = $days;
				$element->business_unit_id = $business_unit_id;
				$element->client_zone_id = $zone->id;

				$filtered = $budget_zones->first(function ($item, $key) use ($zone) {
					return $item->client_zone_id === $zone->id;
				});

				if ( $filtered ) {
					$percentage = ( $filtered->sum_metric_tons * 100 ) / $zones_total_metric_tons;
					$element->metric_tons = ( $totalBudget->metric_tons * $percentage ) / 100;
					$element->total = ( $totalBudget->total * $percentage ) / 100;
					$element->percentage = $percentage;
				} else {
