...
class CommercialChannelReportController extends Controller
{
    public function index() {
		$business_units = BusinessUnit::select('id', 'name')->get();
		$current_date = date(DATE_ATOM, mktime(0, 0, 0));
		return view('backend.commercial_channel_report')->with(compact('business_units', 'current_date'));
	}
	...
	public function list() {
		...
		if ( $export) {
			...
			$sheet->setCellValue('L3', 'Código del Cliente');
			$sheet->setCellValue('M3', 'Razón Social');
			$sheet->setCellValue('N3', 'Articulo');
            $sheet->setCellValue('O3', 'TM');
			...
			$sheet->setCellValue('V3', 'Zona');
			$sheet->setCellValue('W3', 'Distrito');
			$sheet->setCellValue('X3', 'Provincia');
			$sheet->setCellValue('Y3', 'Departamento');
			$sheet->setCellValue('Z3', 'Grupo');
			...
			//	$sheet->setCellValue('L'.$row_number, $element->client_code);
				...
				$sheet->setCellValue('Z'.$row_number, $element->grupo);				
