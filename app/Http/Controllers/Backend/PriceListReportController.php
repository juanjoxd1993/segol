...
class PriceListReportController extends Controller
{
	...
	public function list() {
		...
		$elements = PriceList::leftjoin('clients', 'price_lists.client_id', '=', 'clients.id')
			...
			->select('price_lists.id', 'business_units.name as business_unit_name', 'companies.short_name as company_short_name', 'clients.id as client_id','clients.grupo as client_grupo', 'clients.code as client_code', 'document_types.name as document_type_name', 'clients.document_number', 'clients.business_name', 'articles.code as article_code', 'articles.name as article_name', 'price_lists.price_igv', 'price_lists.initial_effective_date', 'family.name as family_name', 'subgroup.name as subgroup_name', 'client_channels.name as client_channel_name', 'client_zones.name as client_zone_name', 'client_sectors.name as client_sector_name', 'client_routes.short_name as client_route_id', 'client_routes.name as client_route_name')
		...
		if ( $export_excel ) {
			...
			foreach ($elements as $index => $element) {
				...
				$sheet->setCellValue('T'.$row_number, $element->client_grupo);
