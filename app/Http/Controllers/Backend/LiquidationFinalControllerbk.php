		...
		foreach ($sales as $sale) {
			...
			if ( array_key_exists('liquidations', $sale) ) {
				if ( count($sale['liquidations']) > 0 ) {
                    ...
                    //            $sale_model->payment_method_deposit = 2;
                    ...
                    //        $sale_model->save();
                    //    } elseif ($liquidation['payment_id']['id'] == 2) {
                    //       $sale_model->balance -= $liquidation['amount'];
                    //        $sale_model->paid += $liquidation['amount'];

                     //       $sale_model->payment_method_credit = 3;
                         //   $sale_model->pre_balance = round($liquidation['amount'], 4)-round($sale_model['efective'], 4)-round($sale_model['deposit'], 4);
                     //       $sale_model->save();
                    ...
                    //            'amount' => round($liquidation['amount'], 4),
                    ...
                    //    }
						...
						$deposit = Liquidation::sum('amount')
						->where('sale_id', $sale['id'])
						->where('payment_method_id', 2);

				      $sale_model->deposit = $deposit;
					  $sale_model->save();
