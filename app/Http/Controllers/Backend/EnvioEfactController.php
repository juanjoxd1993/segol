...
class EnvioEfactController extends Controller
{
    ...
    public function send_voucher()
    {
        ...
        $vouchers = Voucher::leftjoin('credit_note_reasons', 'credit_note_reasons.id', '=', 'vouchers.credit_note_reason_id')
            ...
            ->leftjoin('ubigeos', 'ubigeos.id', 'client_addresses.ubigeo_id')
            ->leftjoin('address_types', 'address_types.id', 'client_addresses.address_type_id')
            ...
            ->select(
                ...
                'voucher_types.id as voucher_type_id_original',
                ...
                'vouchers.shipping_date as shipping_date',
                'address_types.name as urbanizacion'
            )
        ...
        foreach ($vouchers as $voucher) {
            $voucher->ose = 1;
            $voucher->shipping_date = Carbon::now()->format('Y-m-d');
            $voucher->update();
        }
        ...
        $vouchers->map(function ($item, $key) use ($task, &$response, &$routes, &$mail_info, &$low_date, &$low_number) {
            ...
            if ($task == 'xml') {
                ...
                $res = $this->billingClientPunto->sendDocumentXML(base_path('html/' . $nombre_ruta));
