...
class LiquidationSalesReportController extends Controller
{
    public function index() {
		$companies = Company::select('id', 'name')->get();
		$current_date = date(DATE_ATOM, mktime(0, 0, 0));
		return view('backend.liquidations_sales_report')->with(compact('companies', 'current_date'));
	}
	...
	public function list() {
		...
		if ( $export) {
			...
			$sheet->setCellValue('L3', 'Articulo');
            $sheet->setCellValue('M4', 'TM');
            $sheet->setCellValue('N4', 'Precio');
			$sheet->setCellValue('O3', 'Total');
			$sheet->setCellValue('P3', 'Código del Cliente');
			$sheet->setCellValue('Q3', 'Razón Social');
			$sheet->setCellValue('R3', '# de Parte');
			$sheet->setCellValue('S3', 'Tipo Movimiento');
			$sheet->setCellValue('T3', 'Guía');
			$sheet->setCellValue('U3', 'Placa');
			...
			foreach ($response as $index => $element) {
                ...
                $sheet->setCellValue('L'.$row_number, $element->article_name);
                $sheet->setCellValue('M'.$row_number, $element->sum_total);
				$sheet->setCellValue('N'.$row_number, $element->price); 
				$sheet->setCellValue('O'.$row_number, $element->total);
				$sheet->setCellValue('P'.$row_number, $element->client_code);
				$sheet->setCellValue('Q'.$row_number, $element->client_business_name);
				$sheet->setCellValue('R'.$row_number, $element->warehouse_movement_movement_number);
				$sheet->setCellValue('S'.$row_number, $element->movement_type_name);
				$sheet->setCellValue('T'.$row_number, $element->guide);
