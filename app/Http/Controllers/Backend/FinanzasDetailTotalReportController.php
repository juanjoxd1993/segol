...
class FinanzasDetailTotalReportController extends Controller
{
	...
	public function getState()
	{
		...
		$state = CajaState::select('id', 'state')
			->where('issue_date', $initial_date)->first();
	...
	public function list()
	{
		...
		if ($export == 1) {
			...
			$sheet->setCellValue('A1', 'REPORTE DE LIQUIDACIÓN DEL DÍA ' . $initial_date . ' DESCARGADO EL  ' . CarbonImmutable::now()->format('d/m/Y H:i:s'));
