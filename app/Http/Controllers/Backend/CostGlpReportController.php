	...
	public function list() {
		...
		/*	$sheet->getStyle('Z'. $row_number . ':AB' . $row_number)->applyFromArray([
			...
			$sheet->getStyle('AI'. $row_number . ':AK' . $row_number)->applyFromArray([
				...
				elseif (  $report->udid == '2022052Percy Velaochaga'||$report->udid == '2022062Percy Velaochaga'||$report->udid == '20220721' ||$report->udid == '20220821'||$report->udid == '20220921'||$report->udid == '20221021' ||$report->udid == '20221121') {
				 $tm_fin = SaleDetail::leftjoin('sales', 'sale_details.sale_id', '=', 'sales.id')
				 ->leftjoin('clients', 'sales.client_id', '=', 'clients.id')
				 ->leftjoin('articles', 'sale_details.article_id', '=', 'articles.id')
				 ->where('sales.sale_date', '>=', $mes_date)
			 ...
			 elseif ( $report->udid == '2022042Cesar Coronado'|| $report->udid == '2022052Cesar Coronado'||$report->udid == '2022062Cesar Coronado'||$report->udid == '20220722'||$report->udid == '20220822'||$report->udid == '20220922'||$report->udid == '20221022' ||$report->udid == '20221122') {
				 $tm_fin = SaleDetail::leftjoin('sales', 'sale_details.sale_id', '=', 'sales.id')
				 ->leftjoin('clients', 'sales.client_id', '=', 'clients.id')
				 ->leftjoin('articles', 'sale_details.article_id', '=', 'articles.id')
				 ->where('sales.sale_date', '>=', $mes_date)
			 ...
			 elseif (  $report->udid == '2022042Mauricio Diaz'|| $report->udid == '2022052Mauricio Diaz'||$report->udid == '2022062Mauricio Diaz'||$report->udid == '20220723'||$report->udid == '20220823'||$report->udid == '20220923'||$report->udid == '20221023' ||$report->udid == '20221123' ) {
				 $tm_fin = SaleDetail::leftjoin('sales', 'sale_details.sale_id', '=', 'sales.id')
				 ->leftjoin('clients', 'sales.client_id', '=', 'clients.id')
				 ->leftjoin('articles', 'sale_details.article_id', '=', 'articles.id')
				 ->where('sales.sale_date', '>=', $mes_date)
 ...
 elseif ( $report->udid == '20220410MINERIA'||$report->udid == '20220510MINERIA'||$report->udid == '20220610MINERIA'||$report->udid == '2022071099'||$report->udid == '2022081099'||$report->udid == '2022091099'||$report->udid == '2022101099'||$report->udid == '2022111099'||$report->udid == '2022121099') {
	...
	// <<--------------------------------------------------------------CALCULO DE MB -------------------------------------------------------------------------------->>
					  $movements = WarehouseMovement::select('id', 'warehouse_type_id', 'movement_type_id', 'referral_serie_number', 'referral_voucher_number', 'created_at')
                        ...
                        $total = WarehouseMovement::whereIn('referral_voucher_number', $movements->pluck('referral_voucher_number'))
                       ->where('movement_class_id', 1)
                       ->select('soles')
                       ->sum('soles');

                        $quantity = WarehouseMovementDetail::whereIn('warehouse_movement_id', $movements->pluck('id'))
                       ->select('converted_amount')
                       ->sum('converted_amount');   

                       $report->soles_glp_e +=$total;
                       $report->kgs_glp_e +=$quantity;
