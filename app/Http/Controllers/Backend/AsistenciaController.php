    ...
    public function list()
    {
        ...
        $elements = Employee::leftjoin('asists', 'employees.id', '=', 'asists.employ_id')
            ...
            ->select(
                ...
                'asists.horas_extra_25',
                'asists.horas_extra_35',
                'asists.horas_noc_25',
                'asists.horas_noc_35'
            )
            ->where('employees.state_id', '=', 1) // Condición para empleados activos
            ->when($ciclo, function ($query) use ($ciclo) {
                return $query->where('asists.ciclo_id', $ciclo->id)
                             ->where('asists.año', $ciclo->año)
                             ->where('asists.mes', $ciclo->mes);
            })
            ->when($area_id, function ($query, $area_id) {
                return $query->where('employees.area_id', $area_id);
            })
            ->orderBy('employees.first_name', 'asc') // Ordenar alfabéticamente por nombre
            ->get();
