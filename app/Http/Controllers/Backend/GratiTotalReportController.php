...
class GratiTotalReportController extends Controller
{
	public function index()
	{
		$companies = Company::select('id', 'name')->get();
		$current_date = date(DATE_ATOM, mktime(0, 0, 0));
		$ciclos = Cicle::select('id', 'año', 'mes')->whereIn('mes', [7, 12])->get();


		return view('backend.grati_total_report')->with(compact('companies', 'current_date', 'ciclos'));
	}
	...
	public function list()
	{
		...
		foreach ($elements as $facturation) {
			...
			$total_comp = $facturation->sueldo + $facturation->familiar + $facturation->prom_he + $facturation->prom_bom + $facturation->prom_don + $facturation->prom_noc;
			$facturation->total_comp = round($total_comp, 2);


			$import_mes = ($facturation->total_comp / 6 / 30) * $facturation->dias_total;
			...
			$total_cts = $facturation->import_mes + $facturation->salud;
		...
		*/
		...
		$sheet->getStyle('N2')->applyFromArray([
			...
			$sheet->setCellValue('P3', 'Promedio de Horas Extras');
			$sheet->setCellValue('Q3', 'Bono por Cumplimiento');
			//	$sheet->setCellValue('R3', '1/6 de Feriado');
			$sheet->setCellValue('S3', 'Promedio de Dominical');
			$sheet->setCellValue('T3', 'Promedio de Nocturnidad');
			$sheet->setCellValue('U3', 'Computable Base');
			$sheet->setCellValue('V3', 'Gratificación');
			$sheet->setCellValue('W3', 'Bonificación Extraordinaria Ley 30334');
			...
			foreach ($response as $index => $element) {
				...
				$sheet->setCellValue('K' . $row_number, 'NO');
				...
				$sheet->setCellValue('W' . $row_number, $element->salud);
			...
			$excelStyleService::cabezeraEstilos($sheet, 'FF808000');
