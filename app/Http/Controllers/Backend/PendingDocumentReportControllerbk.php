...
class PendingDocumentReportController extends Controller
{
    public function index() {
		...
		return view('backend.pending_document_report')->with(compact('companies', 'max_datetime', 'warehouse_document_types', 'business_units'));
	}
	...
	public function list() {
		...
		if ( $export ) {
			...
			$sheet->setCellValue('A1', 'DOCUMENTOS PENDIENTES '.$initial_date->format('d/m/Y').' AL '.$final_date->format('d/m/Y'));
			...
			$sheet->setCellValue('P3', 'Pago a cuenta');
			$sheet->setCellValue('Q3', 'Saldo');
			$sheet->setCellValue('R3', 'Undidad Negocio');
			...
			foreach ($response as $index => $element) {
				...
				$sheet->setCellValue('O'.$row_number, $element->total_perception);
				$sheet->setCellValue('P'.$row_number, $element->paid);
				$sheet->setCellValue('Q'.$row_number, $element->balance);
				$sheet->setCellValue('R'.$row_number, $element->business_unit_name);
