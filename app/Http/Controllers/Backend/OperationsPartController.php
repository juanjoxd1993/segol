...
class OperationsPartController extends Controller
{
    public function index() {
		$warehouse_types = WarehouseType::select('id', 'name')->get();
		$movement_classes = MoventClass::select('id', 'name')->get();

		return view('backend.operations_part')->with(compact('warehouse_types', 'movement_classes'));
	}
    ...
    public function list() {
		...
		$traslate_date = date('Y-m-d', strtotime($warehouse_movement->traslate_date));
	...
	public function export() {
		...
		$warehouse_movement = WarehouseMovement::select('id', 'company_id', 'warehouse_type_id', 'movement_class_id', 'movement_type_id', 'movement_number', 'warehouse_account_type_id', 'account_id', 'account_document_number', 'account_name', 'referral_guide_series', 'referral_guide_number',  'scop_number', 'license_plate', 'total', 'created_at')
	...
	public function exportReferralGuide() {
		$pdf = PDF::loadView('backend.pdf.referral_guide2');
		return $pdf->stream();
	}
}
