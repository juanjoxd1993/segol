...
class RegisterDocumentFactController extends Controller
{
    public function index() {
		...
		return view('backend.register_document_fact')->with(compact('companies', 'warehouse_document_types', 'min_sale_date', 'max_sale_date', 'payments', 'min_expiry_date', 'max_expiry_date', 'currencies', 'business_units', 'credit_note_reasons', 'units', 'value_types'));
	}
	...
	public function store() {
		...
		if ( $voucher_type_id ) {
			...
			if ( $voucher_type->id == 3 || $voucher_type->id == 4 ) {
                 $voucher_type->serie_type . sprintf('%02d', $referral_serie_number);
				 $referral_voucher_number;
			} else if ( $voucher_type->id == 7 ||$voucher_type->id == 8 || $voucher_type->id == 11 ||$voucher_type->id == 12 ) {
               $voucher_type->serie_type . sprintf('%03d', $referral_serie_number);
				$referral_voucher_number;
            }
		...
		foreach ($items as $index => $item) {
			...
			if ( $voucher_type_id ) {
				...
				$voucherDetail->concept = $item['concept'];
