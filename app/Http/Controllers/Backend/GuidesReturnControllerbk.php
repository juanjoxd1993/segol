    ...
    public function list() {
        ...
        $movementDetails->map(function ($item, $index) use ($returnWarehouseMovement) {
            ...
            if ( isset($returnWarehouseMovement) ) {
                $returnDetail = WarehouseMovementDetail::select('id', 'article_code', 'new_stock_return')
            ...
            if ( isset($returnDetail) ) {
                ...
                $item->return_converted_amount = number_format($returnDetail->new_stock_return, 4, '.', '');
                $item->balance_converted_amount = number_format($item->converted_amount - $returnDetail->new_stock_return, 4, '.', '');
                $item->new_balance_converted_amount = number_format($item->converted_amount - $returnDetail->new_stock_return, 4, '.', '');
            } else {
