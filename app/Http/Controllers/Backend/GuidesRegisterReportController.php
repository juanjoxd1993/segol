...
use App\Company;
use App\Employee;
use App\Exports\StockRegisterReportExport;
...
class GuidesRegisterReportController extends Controller
{
	public function index() {
		...
		return view('backend.guides_register_report')->with(compact('movement_classes', 'movement_types', 'warehouse_types', 'companies', 'current_date', 'warehouse_account_types', 'warehouse_document_types'));
	}
	...
	public function list() {
		...
		$movements->map(function($item, $index) use($movement_details, $valued, $export) {
			...
			$item->warehouse_movement_details->map(function($detail, $detailIndex) use($item, $account_document_type, $movement_details, $valued, $export) {
				...
				if ( Auth::user()->user == 'operaciones1' || Auth::user()->user == 'global1' || Auth::user()->user == 'admin' ) {
		...
		if ( $export ) {
			$data = new StockRegisterReportExport($movement_details, $valued);
